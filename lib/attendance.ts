...
export async function getAttendanceHistory(
  ...
  const { data, error } = await supabase
    .from('attendance')
    .select('id, scanned_at, event_id, events(title, event_code)')
  ...
  return data.map((row: any) => ({
    ...
    eventTitle: row.events?.title ?? row.events?.event_code ?? 'Event',
...
export async function getTeacherEventAttendance(
  ...
  const { data: attendance, error: attError } = await supabase
    .from('attendance')
    .select('id, student_id, scanned_at, event_id, profiles(full_name, email)')
